...
def calWhiteResult(whiteWindows,symbol,K_MIN,parasetlist,monthlist,datapath,resultpath):
    ...
    for i in np.arange(0, parasetlen):
        ...
        resultdf=pd.read_csv(datapath+filename)
        annual_list=[]
        sharpe_list=[]
        success_rate_list=[]
        drawback_list=[]
        ...
        for i in range(len(monthlist)-whiteWindows):
            ...
            if resultdata.shape[0]>0:
                ...
                annual_list.append(RS.annual_return(resultdata,cash_col='new_own cash',closeutc_col='new_closeutc'))
                sharpe_list.append(RS.sharpe_ratio(resultdata,cash_col='new_own cash',closeutc_col='new_closeutc',retr_col='new_ret_r'))
                success_rate_list.append(RS.success_rate(resultdata,ret_col='new_ret'))
                drawback,a,b=RS.max_drawback(resultdata,cash_col='new_own cash')
...
def calGrayResult(symbol,K_MIN,windowsSet,rankpath,rawdatapath):
    ...
    :return:
    '''
    tf = "%s%s_%d_monthly_retr_3MA.csv" % (rawdatapath, symbol, K_MIN)
...
def getOprlistByMonth(rawpath,symbol,K_MIN,setname,startmonth,endmonth):
    ...
    filename=("%s%d %s 3MAresult3.csv"%(symbol,K_MIN,setname))
    f=rawpath+filename
    oprdf=pd.read_csv(f)
    oprdf=oprdf.loc[(oprdf['openutc'] >= startutc) & (oprdf['openutc'] < endutc)]
    return oprdf[['opentime','openutc','openindex','openprice','new_closetime','new_closeutc','new_closeindex','new_closeprice','tradetype','new_ret','new_ret_r']]
...
def calOprResult(rawpath,symbol,K_MIN):
    ...
    for i in range(graydf.shape[0]):
        gray=graydf.iloc[i]
        oprdf = pd.DataFrame(columns=['opentime', 'openutc', 'openindex', 'openprice', 'new_closetime', 'new_closeutc', 'new_closeindex',
                     'new_closeprice', 'tradetype', 'new_ret', 'new_ret_r'])
        ...
        oprdf.ix[0, 'per earn'] = firsttradecash * oprdf.ix[0, 'new_ret_r']
        # 加入maxcash用于计算最大回撤
        ...
        for i in np.arange(1, oprtimes):
            ...
            perearn = oprdf.ix[i - 1, 'trade money'] * oprdf.ix[i, 'new_ret_r']
        ...
        max_successive_up,max_successive_down=RS.max_successive_up(oprdf,ret_col='new_ret')
        max_return,min_return = RS.max_period_return(oprdf,retr_col='new_ret_r')
...
if __name__ == '__main__':
    ...
    rawdatapath=oprresultpath+'\\ThreeMAClose3to4\\'
    resultpath = rawdatapath+'ForwardResults\\'
    rankpath = rawdatapath+'ForwardRank\\'
