    ...
    for strategyParameter in strategyParameterSet:
        ...
        if multiSTL:
            ...
            if commonForward:
                ...
                indexcolsFlag = False
                getForward(strategyName, symbolinfo, K_MIN, parasetlist, folderpath, startdate, enddate, nextmonth, windowsSet, colslist, positionRatio, initialCash, indexcolsFlag,
                           resultfilesuffix)
            if dsl:
                dslStep = strategyParameter['dslStep']
                stoplossList = np.arange(strategyParameter['dslTargetStart'], strategyParameter['dslTargetEnd'], dslStep)
