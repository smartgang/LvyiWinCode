...
#画买卖点：========================================================================================
def drawOprline(axes,openoprdf,closeoprdf,bottom,top_array,beginindex):
    openlongindex=numpy.array(openoprdf.loc[openoprdf['tradetype']==1]['openindex']-beginindex)
    openshortindex=numpy.array(openoprdf.loc[openoprdf['tradetype']==-1]['openindex']-beginindex)
    closelongindex=numpy.array(closeoprdf.loc[closeoprdf['tradetype']==1]['closeindex']-beginindex)
    closeshortindex=numpy.array(closeoprdf.loc[closeoprdf['tradetype']==-1]['closeindex']-beginindex)
    for i in openlongindex: axes.vlines(i,bottom,top_array[int(i)],edgecolor=__color_gold__, linewidth=1,alpha=0.5)
    for i in openshortindex: axes.vlines(i,bottom,top_array[int(i)],edgecolor=__color_gold__, linewidth=1,alpha=0.5)
    for i in closelongindex: axes.vlines(i,bottom,top_array[int(i)],edgecolor=__color_lightblue__, linestyles='dashed',linewidth=1, alpha=0.5)
    for i in closeshortindex: axes.vlines(i,bottom,top_array[int(i)],edgecolor=__color_lightblue__, linestyles='dashed',linewidth=1,alpha=0.5)
